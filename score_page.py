...
class ScorePage(QWidget):
    def __init__(self, username=None, data_interface=None):
        ...
        # 数据接口，可替换为真实接口
        self.data_interface = data_interface or self._get_real_data_interface
    ...
    def _get_real_data_interface(self):
        """使用database.py中的方法获取真实的历史数据"""
        if not HAS_DATABASE:
            logger.warning("数据库不可用，使用模拟数据")
            return self._mock_data_interface()

        try:
            # 创建数据库连接
            store = TestTableStore(host="localhost", user="root", password="123456", database="test")

            # 使用database.py中的方法获取用户历史数据
            real_data = store.get_user_history_data(self.username)
            # 合并真实数据和模拟数据
            return {
                ...
                "舒尔特准确率": real_data["舒尔特准确率"],
                "舒尔特综合得分": real_data["舒尔特综合得分"],
                "历史": {
                    "疲劳检测": [random.randint(0, 100) for _ in range(max(1, len(real_data['历史']['血压脉搏'])))],
                    "情绪": [random.randint(0, 100) for _ in range(max(1, len(real_data['历史']['血压脉搏'])))],
                    "血压脉搏": real_data["历史"]["血压脉搏"],
                    "脑负荷": [random.randint(0, 100) for _ in range(max(1, len(real_data['历史']['血压脉搏'])))],
                    "舒尔特准确率": real_data["历史"]["舒尔特准确率"],
                },
                "历史日期": real_data.get("历史日期", [])  # 获取历史日期
            }
